    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 text-sm text-gray-600 dark:text-gray-400">
        {property.ratings?.average > 0 && (
          <span className="flex items-center gap-1"><Star size={14} className="fill-gray-900 dark:fill-gray-100 text-gray-900 dark:text-white" /> {property.ratings.average} ({property.ratings.count} reviews)</span>
        )}
      ...
      <div className="relative rounded-2xl overflow-hidden mb-8 bg-gray-100 dark:bg-gray-700/50">
        ...
        {images.length > 1 && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {images.map((_, i) => (
              <button key={i} onClick={() => setImgIdx(i)} className={`w-2 h-2 rounded-full transition-all ${i === imgIdx ? 'bg-white dark:bg-gray-900 scale-125' : 'bg-white dark:bg-gray-900/60'}`} />
            ))}
          </div>
        )}
      </div>
                  ...
                  <div key={r._id} className="border-b border-gray-100 dark:border-gray-700/50 pb-4 last:border-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="ml-auto flex items-center gap-1 text-sm">
                        <Star size={13} className="fill-gray-900 dark:fill-gray-100 text-gray-900 dark:text-white" /> {r.ratings?.overall}
                      </div>
                    </div>

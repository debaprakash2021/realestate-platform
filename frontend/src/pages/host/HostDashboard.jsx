...
const STATUS_STYLES = {
  ...
  rejected:  { cls: 'bg-gray-100 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400',     icon: <AlertCircle size={12} /> }
}
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Your properties, bookings & earnings</p>
        </div>
      ...
      <div className="card p-6 mb-6">
        ...
        {recentBookings.length === 0 ? (
          <p className="text-center py-10 text-sm text-gray-400 dark:text-gray-500">No bookings yet — list your first property!</p>
        ) : (
          <div className="space-y-3">
            {recentBookings.map(b => {
              ...
              return (
                <div key={b._id} className="border border-gray-100 dark:border-gray-700/50 rounded-xl p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  <div className="flex flex-col sm:flex-row gap-4 items-start">
                    ...
                    <div className="flex gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-sm text-gray-900 dark:text-white truncate">{b.property?.title}</p>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{b.guests?.total} guest{b.guests?.total > 1 ? 's' : ''}</p>
                    </div>
                    ...
                    <div className="flex flex-row sm:flex-col items-center sm:items-end gap-2 shrink-0">
                      ...
                      <p className="text-sm font-bold text-gray-900 dark:text-white">₹{b.pricing?.totalAmount?.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {analyticsBook && (
          <div className="card p-6">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-4">Avg stay: {analyticsBook.avgNights} nights</p>
          </div>
        )}
          ...
          <div className="card p-6">
            ...
            {revenue.revenueData.length === 0 ? (
              ...
              <div key={d.month} className="flex items-center gap-3 mb-3">
                <span className="text-sm text-gray-600 dark:text-gray-400 w-20">{d.month}</span>
                <div className="flex-1 bg-gray-100 dark:bg-gray-700/50 rounded-full h-2">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50 flex justify-between text-sm">
      ...
      <div className="card p-6">
        ...
        {properties.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-gray-400 dark:text-gray-500 text-sm mb-4">No properties listed yet</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700/50 text-left text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {properties.map(p => (
                  <tr key={p.propertyId} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    <td className="py-3 font-medium max-w-[200px] truncate">{p.title}</td>
                    <td className="py-3 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${p.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400'}`}>{p.status}</span>
                    </td>
                    <td className="py-3 text-center text-gray-600 dark:text-gray-400">{p.bookingCount}</td>
                    <td className="py-3 text-center text-gray-600 dark:text-gray-400">{p.rating > 0 ? `⭐ ${p.rating}` : '—'}</td>

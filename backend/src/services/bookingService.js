...
class BookingService {
  ...
  static calculateRefund(booking) {
    ...
    const daysToCheckIn = Math.ceil((new Date(booking.checkIn) - new Date()) / (1000 * 60 * 60 * 24));
    let refundAmount    = 0;
    if (policy === 'flexible') refundAmount = daysToCheckIn >= 1 ? totalAmount : 0;
    if (policy === 'moderate') refundAmount = daysToCheckIn >= 5 ? totalAmount : 0;
    if (policy === 'strict')   refundAmount = daysToCheckIn >= 7 ? totalAmount * 0.5 : 0;
    return Math.round(refundAmount * 100) / 100;
  }

  // ─── Create Booking + Fire Notifications ─────────────────────────
  static async createBooking(guestId, data) {
    ...
    const [property, guest] = await Promise.all([
      Property.findById(propertyId).populate('host'),
      User.findById(guestId).select('name email phone avatar')
    ]);
    ...
    if (nights < property.houseRules.minNights) throw new Error(`Minimum stay is ${property.houseRules.minNights} night(s)`);
    if (nights > property.houseRules.maxNights) throw new Error(`Maximum stay is ${property.houseRules.maxNights} night(s)`);

    const totalGuests = (guests?.adults || 1) + (guests?.children || 0);
    if (totalGuests > property.details.maxGuests) throw new Error(`Maximum ${property.details.maxGuests} guests allowed`);

    const hasConflict = await Booking.hasConflict(propertyId, checkIn, checkOut);
    if (hasConflict) throw new Error('Property is not available for the selected dates');
    if (!property.isAvailable(checkIn, checkOut)) throw new Error('Property is blocked for the selected dates');

    const pricing = this.calculatePricing(property, checkIn, checkOut);

    const booking = await Booking.create({
      property: propertyId, guest: guestId, host: property.host._id,
      checkIn: checkInDate, checkOut: checkOutDate, nights,
      guests: { adults: guests?.adults || 1, children: guests?.children || 0, infants: guests?.infants || 0, total: totalGuests },
      pricing,
    ...
    await Property.findByIdAndUpdate(propertyId, {
      $push: { blockedDates: { startDate: checkInDate, endDate: checkOutDate, reason: 'booked' } },
    ...
    await Notification.create({
      ...
      title:   `📅 New Booking: ${property.title}`,
      message: `${guest.name} (${guest.email}${guestPhone}) has booked your property "${property.title}" · ${nights} night${nights > 1 ? 's' : ''} · ${fmt(checkInDate)} → ${fmt(checkOutDate)} · ${totalGuests} guest${totalGuests > 1 ? 's' : ''} · ₹${pricing.totalAmount.toLocaleString()} · Payment pending`,
    ...
    await Notification.create({
      user:    guestId,
      type:    'booking_confirmed',
      title:   `🎉 Booking Confirmed: ${property.title}`,
      message: `Your booking for "${property.title}" · ${fmt(checkInDate)} → ${fmt(checkOutDate)} · ${nights} night${nights > 1 ? 's' : ''} · Total ₹${pricing.totalAmount.toLocaleString()}. Please complete payment to confirm your stay.`,
      data:    { bookingId: booking._id, propertyId }
    });

    return await booking.populate([
      { path: 'property', select: 'title location images pricing' },
      { path: 'guest',    select: 'name email avatar phone' },
  ...
  static async rejectBooking(bookingId, hostId, reason) {
    ...
    await Property.findByIdAndUpdate(booking.property, {
      $pull: { blockedDates: { startDate: booking.checkIn, endDate: booking.checkOut } }
    });
    await booking.save();
    return booking;
  }
  ...
  static async cancelBooking(bookingId, userId, reason) {
    ...
    if (!['pending', 'confirmed'].includes(booking.status)) throw new Error('This booking cannot be cancelled');
  ...
  static async completeBooking(bookingId) {
    ...
    await User.findByIdAndUpdate(booking.host, {
      $inc: { 'hostInfo.totalEarnings': booking.pricing.totalAmount, 'stats.totalBookings': 1 }
    });
    await booking.save();
    return booking;
  }

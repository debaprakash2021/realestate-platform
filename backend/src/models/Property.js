...
propertySchema.virtual('weeklyPrice').get(function() {
  const discount = this.pricing.weeklyDiscount / 100;
  return Math.round(this.pricing.basePrice * 7 * (1 - discount));
});

// ─── Virtual: Price with Monthly Discount ────────────────────────
propertySchema.virtual('monthlyPrice').get(function() {
  const discount = this.pricing.monthlyDiscount / 100;
  return Math.round(this.pricing.basePrice * 30 * (1 - discount));
});
